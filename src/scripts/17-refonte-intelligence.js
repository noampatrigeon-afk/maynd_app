...
var LENGTH_LEVELS={
  ...
  2:{words:90,  tokens:190, instruction:"réponds en un petit paragraphe : un seul angle, et cherche activement une piste concrète que la personne peut vraiment essayer — ne t'arrête à la seule réflexion que si aucune piste sincère ne se présente"},
  3:{words:180, tokens:400, instruction:"tu peux développer davantage : articule deux idées et une piste concrète et détaillée à essayer — comprendre ne suffit pas quand un pas concret existe, propose-le clairement plutôt que de rester sur le seul constat"},
  4:{words:99999,tokens:1200,instruction:"la personne demande du fond : tu peux développer pleinement, jusqu'à ta longueur habituelle, et si un pas concret se dessine, propose-le sans le diluer dans l'explication"}
};
/* Retour utilisateur du 25/09/2026 : les accompagnants décelaient bien les problèmes mais ne
   proposaient presque jamais rien de concret derrière. Diagnostic : mécanique, pas bug — les
   niveaux 2 et 3 ne mentionnaient une piste que sous condition molle ("si elle sert vraiment",
   "si utile"), qui laissait la porte grande ouverte au silence. Renforcé sur demande explicite,
   "toujours au jugement" : la piste devient la sortie par défaut d'un échange installé, sauf
   qu'aucune ne s'impose sincèrement — jamais forcée quand ça n'a pas de sens, mais plus proposée
   par réflexe d'écoute seule. Le niveau 1 (ouverture) n'est pas touché : forcer une action dès le
   tout premier échange contredirait l'esprit du reste du fichier (MIA, par exemple, n'oriente
   jamais dans sa toute première réponse sauf risque). */
...
/* Retour utilisateur du 25/09/2026 : le message discret ("ça se découvre au fil du parcours")
   laissait croire à un bug, y compris pour une personne payante — puisque le multi-accompagnants
   n'est justement plus lié au palier. Demande explicite : un message net, avec un bouton qui
   débloque vraiment (la seule mécanique de déblocage qui existe tant que la progression n'est pas
   construite), plutôt qu'un bouton mort. Vocabulaire « débloqué » assumé ici en connaissance de
   cause : c'est une dérogation ponctuelle et volontaire à la règle du chantier 12 pour cet écran
   précis, pas un retour en arrière général sur les accompagnants ou les formules. */
var _mixPendingId=null;
function openMixSheet(id){
  _mixPendingId=id;
  var a=id?byId(id):null;
  $('mix-sub').textContent=(a?a.name:'Cet accompagnant')+' rejoindra la conversation dès que le multi-accompagnants sera débloqué sur ce compte.';
  $('mix-cta').textContent='Débloquer'+(a?' '+a.name:'');
  openSheet('mix-backdrop','mix-sheet');
}
function mixUnlockGo(){
  closeSheet('mix-backdrop','mix-sheet');
  state.multiUnlocked=true; persist();
  if(typeof renderProfile==='function' && typeof isOpen==='function' && isOpen('profile-sheet')) renderProfile();
  var id=_mixPendingId; _mixPendingId=null;
  if(id) addParticipant(id);
}

/* inviteEl redéfinie : deuxième chemin par lequel le blocage multi-accompagnants apparaissait,
   raté à la première passe du chantier 12. Quand MIA propose elle-même de faire venir un
   accompagnant sans que canUseMulti() soit vrai, handleJoin passe par addInvite/inviteEl plutôt
   que par addParticipant — la carte affichait encore « · MAYND+ » (palier disparu) et son clic
   ouvrait l'écran de formules (payer ne change plus rien ici). Le clic route maintenant par
   addParticipant, qui contient déjà toute la bonne logique de blocage (agent non accessible,
   multi non débloqué -> openMixSheet, cap de 3 déjà atteint) : une seule vérité, pas de duplication. */
function inviteEl(agentId){
  const a=byId(agentId);
  const couldHelp={fr:"pourrait t'aider",en:"could help",es:"podría ayudarte"}[state.lang]||"pourrait t'aider";
  const d=document.createElement('div'); d.className='bubble note'; d.style.cursor='pointer';
  d.onclick=function(){ addParticipant(agentId); };
  d.innerHTML='<span class="nd" style="background:'+a.color+'">'+a.name[0]+'</span><span>'+a.name+' '+couldHelp+'</span>';
  return d;
}
